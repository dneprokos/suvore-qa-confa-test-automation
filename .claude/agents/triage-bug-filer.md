...
- `Defect Detection Phase` is `customfield_10203` and it is **required**. Jira fills it with `Development`
  ...
  no error anywhere.
- `Bash` is granted for one command: `node .claude/skills/jira-bug-creator/scripts/draft-bug.js`. Nothing
...
```bash
node .claude/skills/jira-bug-creator/scripts/draft-bug.js --draft "<draft_path>" --json
```
