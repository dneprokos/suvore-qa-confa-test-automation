...
- The draft builder is `node .claude/skills/jira-bug-creator/scripts/draft-bug.js`. It is the only way you
...
- `Bash` is granted for `node .claude/skills/jira-bug-creator/scripts/draft-bug.js` and for the `cp` that
  immediately follows it. Run nothing else — not the test suite, not the type check, not git, not a
  package install.

# Step 1 — Read the contract

`Read` `docs/automation/contracts/slack-bug-intake.md` in full before you draft anything.
...
```bash
node .claude/skills/jira-bug-creator/scripts/draft-bug.js --manual --json \
  --set summary="…" \
  --set steps+="…" --set steps+="…" \
