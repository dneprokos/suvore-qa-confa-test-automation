...
Opening a PR needs a GitHub token with the **same resolution** as **git-pr-creator**: `GITHUB_TOKEN` or `GH_TOKEN`, then **repo-root** `github-pr.local.json` (preferred for Copilot + Cursor skill layouts), then legacy `.claude/skills/git-pr-creator/config/github-pr.local.json`. Details: [git-pr-creator README](../git-pr-creator/README.md).
...
- Use existing scripts under `.claude/skills/*/scripts/`; do not reimplement git operations in the orchestrator narrative.
