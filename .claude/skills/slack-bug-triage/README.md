...
- `scripts/slack-triage-journal.mjs` is the only thing that may write it. Never hand-edit it.
...
`.claude/skills/jira-bug-creator/scripts/lib.js` demands nine fields and treats all nine alike. A person
...
Full rules: `docs/automation/contracts/slack-bug-intake.md`.
...
Full rules: `docs/automation/contracts/triage-verdict-contract.md`.
...
repo's `.env` is read only by `framework/configuration/config.ts`.
...
Details, scopes and the reactions probe: `references/slack-mcp.md`.
...
```
.claude/skills/slack-bug-triage/
...
.claude/agents/triage-*.md          the five steps — one file each, none naming another
scripts/slack-triage-journal.mjs    the only writer of the ledger; owns the skip filter
scripts/__tests__/slack-triage-journal.test.mjs   run with `npm run test:scripts`
docs/automation/contracts/slack-bug-intake.md         message -> draft; read by the two steps that must agree
docs/automation/contracts/triage-verdict-contract.md  duplicate -> route; same reason
