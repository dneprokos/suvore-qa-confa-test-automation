...
/**
 * Project root. Inside a plugin the scripts do not live under the repository they write to, so the root
 * comes from the environment Claude Code sets, and from the working directory when run by hand.
 */
const REPO_ROOT = path.resolve(process.env.CLAUDE_PROJECT_DIR || process.cwd());
/** config.json ships beside these scripts in the plugin, not one level up as in the skill layout. */
